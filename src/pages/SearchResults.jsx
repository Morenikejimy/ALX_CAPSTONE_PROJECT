      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        {!query && !loading && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500">Enter a book title or author name to find amazing books!</p>
          </div>
        )}
          ...
          <>
            <div className="mb-4">
              <p className="text-gray-600">
                Found <span className="font-bold text-gray-800">{books.length}</span> {books.length === 1 ? 'book' : 'books'}
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {books.map((book) => (
                <Link
                  to={`/book-details/${book.categoryId}/${book.id}`}
                  key={book.id}
